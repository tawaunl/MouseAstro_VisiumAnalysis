...
design <- model.matrix(~0+factor(Genotype), y$samples)
colnames(design) <- c("TauPS2APP","NonTG")
contr <- makeContrasts(NonTG-TauPS2APP,levels=design ) # NonTG as reference
y <- estimateDisp(y, design)

fit <- glmQLFit(y, design, robust=TRUE)
st_de <- glmQLFTest(fit, coef=ncol(design), contrast = contr)
st_de <- data.frame(topTags(st_de, n=50000,p.value = 10))

## =================Volcano Plot for AD data=========================
library(scales)
st_de$diffexpressed <- "unchanged"

st_de$diffexpressed[st_de$logFC >= 0.5 &
                      st_de$FDR<=0.05 ] <- "TauPS2APP"

st_de$diffexpressed[st_de$logFC <= -0.5 &
                      st_de$FDR<=0.05] <- "NonTG"

mycolors <- c("red", "blue", "black")
names(mycolors) <- c("TauPS2APP", "NonTG", "unchanged")

png(file.path(RESULTS_PATH,"Volcano_TauPS2APPvsNonTG.png"),
    width =600,height = 500)
ggplot(data=st_de, aes(x=logFC, y=-log10(PValue), col=diffexpressed, label=rownames(st_de))) + 
  geom_point(size=2) + 
  theme_classic()+ geom_label_repel(aes(size=8),max.overlaps=2) +
  scale_colour_manual(values = mycolors) + ggtitle("PseudoBulk Spots: TauPS2APP vs NonTG")+
...
# Puesdobulk DE Pathology vs No ALL Analysis =================================
sum_by <- c("Abeta","orig.ident")
summed <- aggregateAcrossCells(se, id=colData(se)[,sum_by])
...
design <- model.matrix(~0+factor(Abeta), y$samples)
...
p <- ggplot(data=AllPath_de, aes(x=logFC, y=-log10(PValue), col=diffexpressed, label=rownames(AllPath_de))) + 
  geom_point(size=2) + 
  theme_classic() +
  scale_colour_manual(values = mycolors) + ggtitle("PseudoBulk: Pathology vs No Pathology All Spots")+
  theme(legend.text = element_text(size=16,face = "bold"),
        legend.title = element_text(size=18,face='bold'),
        legend.key.size = unit(1 ,'cm'),
        axis.title = element_text(size=20,face='bold'),
...
png(file.path(RESULTS_PATH,"Volcano_AllNoPathologyvsNoPathology.png"),
...
png(file.path(RESULTS_PATH,"Volcano_TauPS2APPNoPathologyvsNoPathology_AllRegions.png"),
    width =1000,height = 800)
ggplot(data=AllPath_de, aes(x=logFC, y=-log10(PValue), col=diffexpressed, label=rownames(AllPath_de))) + 
  ...
  scale_colour_manual(values = mycolors) + ggtitle("PseudoBulk: TauPS2APP Pathology vs No Pathology All Spots")+
  theme(legend.text = element_text(size=16,face = "bold"),
        legend.title = element_text(size=18,face='bold'),
        legend.key.size = unit(1 ,'cm'),
        axis.title = element_text(size=20,face='bold'),
...
write.csv(AllPath_de,file.path(RESULTS_PATH,"TauPS2APP_DE_PathologyvsNoPathology_AllRegions.csv"))
...
png(file.path(RESULTS_PATH,"Volcano_TauPS2APPNoPathologyvsNoPathology_Cortex.png"),
    width =1000,height = 800)
ggplot(data=AllPath_de, aes(x=logFC, y=-log10(PValue), col=diffexpressed, label=rownames(AllPath_de))) + 
  geom_point(size=2) + 
  theme_classic()+ geom_text_repel(aes(size=8),max.overlaps=2) +
  scale_colour_manual(values = mycolors) + ggtitle("PseudoBulk: TauPS2APP Pathology vs No Pathology Cortex")+
  theme(legend.text = element_text(size=16,face = "bold"),
        legend.title = element_text(size=18,face='bold'),
        legend.key.size = unit(1 ,'cm'),
        axis.title = element_text(size=20,face='bold'),
...
write.csv(AllPath_de,file.path(RESULTS_PATH,"TauPS2APP_DE_PathologyvsNoPathology_Cortex.csv"))
...
write.csv(AllPath_de,file.path(RESULTS_PATH,"TauPS2APP_DE_PathologyvsNoPathology_Hippocampus.csv"))

# 4 way plot Hipp vs Cortex ================================
library(gg4way)
hipp <- read.csv(file.path(RESULTS_PATH,"TauPS2APP_DE_PathologyvsNoPathology_Hippocampus.csv"))
cortex <- read.csv(file.path(RESULTS_PATH,"TauPS2APP_DE_PathologyvsNoPathology_Cortex.csv"))
...
gg4way(x,
  ...
  xlab(expression(atop(
    paste("Higher in No Pathology" %<->% "Higher in Pathology"),
    paste("Cortex LogFC")))) +
...
names(sobj_merge@meta.data) <-  str_replace_all(x,"Module\\d+","")
...
coldata_short <- coldata %>% dplyr::select(Sample,Abeta,Condition,Homeostatic,DAA1,DAA2,Synapse,BrainRegion) %>% unique


df_long_final <- left_join(norm_counts,coldata_short, by="Sample") 

plotdata <- aggregate(cbind(Homeostatic,DAA1,DAA2,Synapse) ~Sample + BrainRegion +Condition+Abeta, data = df_long_final, median, na.rm = TRUE)
data <- pivot_longer(plotdata,cols = c(Homeostatic,DAA1,DAA2,Synapse))
data_tau <- data[data$Condition=="TauPS2APP",]

data_filtered <- data[-c(1:8),] # remove all nontg Abeta 
data_filtered <- data_filtered[!c(data_filtered$name=="Synapse"),]

ggplot(data_filtered, aes(x=Abeta, y=value,fill=Abeta)) + theme_classic() +
  geom_boxplot(outlier.shape=NA) + scale_fill_viridis(discrete = TRUE, alpha=1) +
  geom_jitter(aes(color=Condition),width= 0.1,size=2) + 
  facet_wrap(factor(BrainRegion)~factor(name),scales = "free", ncol=3) + xlab("Pathology") +
  ...
  theme(strip.text.x = element_text(size = 20,face = "bold"),
        ...
        legend.title = element_text(size=16)) + geom_line(aes(group = Sample), alpha = 0.6, colour = "black", data = data_filtered)

##  Puesdobulk =======================
pBulk_all <- AggregateExpression(sobj_merge, group.by = c("orig.ident","Abeta","BrainRegion","Condition"),return.seurat=TRUE,assays = "SCT")

gois <- c(genes_homeo[1:2],genes_DAA1[1:2],genes_DAA2[1:2])

VlnPlot(pBulk_all,features = gois,group.by = "BrainRegion",split.by = "Condition",
       ncol = 6)+
  theme(legend.position = "right") 

VlnPlot(pBulk_all,features = c("Aldoc","Clu"),group.by = "BrainRegion",
        split.by = "Abeta",cols = viridis(2))+theme(legend.position = "right")
VlnPlot(pBulk_all,features = c("Aldoc","Clu"),group.by = "BrainRegion",
...
ggplot(data_filtered, aes(x=Abeta_centers, y=value,fill=Abeta_centers)) + theme_classic() +
  geom_boxplot(outlier.shape=NA) + scale_fill_viridis(discrete = TRUE, alpha=1) +
  geom_jitter(aes(color=Condition),width= 0.1,size=2) + 
  facet_wrap(factor(BrainRegion)~factor(name),scales = "free", ncol=3) + xlab("Pathology") +
  theme(axis.text.x = element_text(angle=75,vjust = 0.5),axis.text.y = element_text(size=20)) +
...
##  Puesdobulk =======================
pBulk_all <- AggregateExpression(sobj_merge, group.by = c("orig.ident","Abeta_centers","BrainRegion","Condition"),return.seurat=TRUE,assays = "SCT")
...
VlnPlot(pBulk_all,features = gois,group.by = "BrainRegion",split.by = "Condition",
        ncol = 6)+
  theme(legend.position = "right") 
VlnPlot(pBulk_all,features = gois,group.by = "BrainRegion",split.by = "Abeta_centers",
        cols=viridis(2),ncol = 6)+
  theme(legend.position = "right") 

VlnPlot(pBulk_all,features = c("Aldoc","Clu"),group.by = "BrainRegion",
        split.by = "Abeta_centers",cols = viridis(2))+theme(legend.position = "right")
VlnPlot(pBulk_all,features = c("Aldoc","Clu"),group.by = "BrainRegion",
        split.by = "Condition")+theme(legend.position = "right")
